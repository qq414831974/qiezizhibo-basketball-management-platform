...
import TeamSimpleForm from "./BasketballTeamModifyDialog";
import {getTeamById, updateTeamById} from "../../../axios";
import TeamAddPlayersPanel from "./BasketballTeamAddPlayersPanel";

const TabPane = Tabs.TabPane;


class BasketballTeamDetailManagement extends React.Component {
    state = {
        currentTab: "1",
        pageLoaded: false,
        data: {},
    ...
    render() {
        if (!(this.props.match.params && this.props.match.params.id)) {
            return <Redirect push to="/basketball/basketballTeam"/>;
        }
        const TeamSimple = Form.create()(TeamSimpleForm);
        return this.state.pageLoaded ? (
            <div className="gutter-div">
                <BreadcrumbCustom first={<Link to={'/basketball/basketballTeam'}>球队管理</Link>} second="详细设置"/>
                <Row gutter={16}>
                    <Col className="gutter-row">
                        <div className="gutter-box">
                            <Card bordered={false}>
                                <div className="inline-block">
                                    <Link to={'/basketball/basketballTeam'}>
...
export default connect(mapStateToProps, mapDispatchToProps)(BasketballTeamDetailManagement);